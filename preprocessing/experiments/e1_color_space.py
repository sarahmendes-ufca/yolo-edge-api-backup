...
if __name__ == "__main__":
    ...
    results.append(
        evaluate_pipeline(preproc_rgb_correct, "E1-B: BGR→RGB (cvtColor)")
    )
    results.append(
        evaluate_pipeline(preproc_rgb_flip, "E1-C: BGR→RGB (NumPy flip)")
    )
    ...
    for r in results[1:]:
        ...
        print(
            f"  {r['label']:35s}  mAP@0.5={r['map50']:.4f}  delta={sinal}{delta:.4f}"
        )
